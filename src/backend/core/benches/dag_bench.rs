...
//!
//! Covers: construction, dependency insertion, topological sort,
//! ready-task discovery, status transitions, cascading cancellation,
//! and agent swarm scaling scenarios (100 / 1,000 / 10,000 agents).
...
fn build_linear_dag(n: usize) -> (TaskDAG, Vec<TaskId>) {
    ...
    for i in 0..n {
        ...
        if let Some(&prev) = ids.last() {
            dag.add_dependency(prev, id).unwrap();
        }
        ids.push(id);
    }
    (dag, ids)
    ...
    for size in [10, 100, 1_000, 10_000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, &n| {
            b.iter(|| {
                ...
                for i in 0..n {
                    let task = Task::new(format!("t-{i}"), TaskInput::default());
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, &n| {
            b.iter_batched(
                || {
                    let mut dag = TaskDAG::new("bench");
                    let ids: Vec<_> = (0..n)
                        .map(|i| dag.add_task(Task::new(format!("t-{i}"), TaskInput::default())).unwrap())
                        .collect();
                    (dag, ids)
                },
                |(mut dag, ids)| {
...
fn bench_dag_topological_sort(c: &mut Criterion) {
    ...
    for size in [10, 100, 1_000] {
        group.bench_with_input(BenchmarkId::new("linear", size), &size, |b, &n| {
            let (dag, _) = build_linear_dag(n);
            b.iter(|| black_box(dag.topological_order().unwrap()));
        });
    }
    for fan in [10, 100, 1_000] {
        group.bench_with_input(BenchmarkId::new("fanout", fan), &fan, |b, &n| {
            let (dag, _) = build_fanout_dag(n);
            b.iter(|| black_box(dag.topological_order().unwrap()));
        });
    }
    for (layers, width) in [(5, 20), (10, 10), (20, 5)] {
        ...
        group.bench_with_input(BenchmarkId::new("layered", &label), &(layers, width), |b, &(l, w)| {
            let (dag, _) = build_layered_dag(l, w);
            b.iter(|| black_box(dag.topological_order().unwrap()));
        });
    }
    group.finish();
...
fn bench_dag_status_transitions(c: &mut Criterion) {
    c.bench_function("dag_status_transitions", |b| {
        b.iter_batched(
            || {
                let mut dag = TaskDAG::new("bench");
                let id = dag.add_task(Task::new("task", TaskInput::default())).unwrap();
                (dag, id)
            },
            |(mut dag, id)| {
                dag.update_task_status(id, TaskStatus::Ready).unwrap();
                dag.update_task_status(id, TaskStatus::Running).unwrap();
                dag.update_task_status(id, TaskStatus::Completed).unwrap();
                black_box(&dag);
            },
            criterion::BatchSize::SmallInput,
        );
...
fn bench_dag_cascading_cancel(c: &mut Criterion) {
    ...
    for depth in [10, 100, 1_000] {
        group.bench_with_input(BenchmarkId::from_parameter(depth), &depth, |b, &n| {
            b.iter_batched(
                || build_linear_dag(n),
                |(mut dag, ids)| { black_box(dag.cancel_dependents(ids[0]).unwrap()); },
                criterion::BatchSize::SmallInput,
            );
        });
    }
...
fn bench_dag_agent_scenarios(c: &mut Criterion) {
    ...
    for agents in [100, 1_000, 10_000] {
        ...
        group.bench_with_input(BenchmarkId::new("construct_linear", agents), &agents, |b, &n| {
            b.iter(|| black_box(build_linear_dag(n)));
        });
        group.bench_with_input(BenchmarkId::new("construct_fanout", agents), &agents, |b, &n| {
            b.iter(|| black_box(build_fanout_dag(n)));
        });
        let width = (agents as f64).sqrt() as usize;
        let layers = agents / width;
        group.bench_with_input(BenchmarkId::new("construct_layered", agents), &(layers, width), |b, &(l, w)| {
            b.iter(|| black_box(build_layered_dag(l, w)));
        });
    }
    group.finish();
}
...
criterion_group!(
    ...
    bench_dag_dependency_insertion,
    bench_dag_topological_sort,
    bench_dag_ready_tasks,
    bench_dag_status_transitions,
    bench_dag_cascading_cancel,
    bench_dag_stats,
    bench_dag_agent_scenarios,
);
